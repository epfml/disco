...
function parseAnswerLine(line: string): { label: string; answer?: string } | undefined {
    ...
    return {
        label,
        answer: answerText === undefined || answerText === "" ? undefined : `${label}. ${answerText}`,
    };
}
...
function parseSample(sample: string): ParsedSample {
    ...
    if (correctOption === undefined) {
        throw new Error(`Could not match answer label ${JSON.stringify(answerLabel)} to an option`);
    }
...
async function benchmarkFullAnswers(
    ...
    for (const sample of dataset) {
        ...
        try {
            ...
            console.log("Invalid sample:", error instanceof Error ? error.message : error);
            continue;
        }

        if (!validateOptions(parsed.options, labels)) {
            console.log("Invalid options:", parsed.options.map((option) => option.label).join(", "));
            continue;
        }
        ...
        logs.push({
            ...
            continuationTokens: Object.fromEntries(
                parsed.options.map((option, i) => [option.label, results[i].continuationTokens]),
            ),
            usedInputTokens: Object.fromEntries(
                parsed.options.map((option, i) => [option.label, results[i].usedInputTokens]),
            ),
        });
...
async function main() {
    ...
    for (const format of formats) {
        ...
        await benchmarkFullAnswers(model, tokenizer, dataset, format, contextLength, savePath);
    }
