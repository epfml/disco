  ...
  <div v-if="workingModelExistsOnMount">
    <IconCard>
      ...
      <template #content>
        ...
        <div
          v-if="
            memoryStore.useIndexedDB && workingModelExists && !isModelCreated
          "
        >
          <div class="grid grid-cols-4 items-center justify-items-center">
            <div class="col-span-3">
              <div class="text-sm text-gray-500 dark:text-light">
                Disco has cached the last model you were working on for you.
                Select it to start training from it. Otherwise, it will be
                overridden the next time you train the
                {{ task.displayInformation.taskTitle }} task.<br />
          ...
          <div class="flex pt-4 space-x-4 justify-center">
            <button
              class="flex items-center justify-center px-4 py-2 space-x-4 transition-colors border rounded-md text-gray-900 border-gray-900"
              @click="saveModel()"
            >
        ...
        <div v-else class="text-sm text-gray-500">
          ...
          <div v-else-if="!memoryStore.useIndexedDB && workingModelExists">
            FeAI cached the last model you were working on for you. Turn on the
            model library (see settings) to see additional options.
          </div>
...
export default {
  ...
  computed: {
    ...
    memory(): Memory {
      return this.memoryStore.useIndexedDB
        ? new browser.IndexedDB()
        : new EmptyMemory();
    },
    modelInfo(): ModelInfo {
      return {
