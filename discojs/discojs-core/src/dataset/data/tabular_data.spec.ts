...
describe('tabular data checks', () => {
    ...
    const columnConfigs = Map(
        Set(dataConfig.features).map((feature) => [
            feature,
            { required: false, isLabel: false },
        ])
    ).merge(
        Set(dataConfig.labels).map((label) => [
            label,
            { required: true, isLabel: true },
        ])
    )
    ...
    it('throw an error on incorrectly formatted data', async () => {
        try {
            await TabularData.init(
                tf.data.csv(
                    'file://../../example_training_data/cifar10-labels.csv',
                    csvConfig
                ),
                titanicMock,
                3
    ...
    it('do nothing on correctly formatted data', async () => {
        await TabularData.init(
            tf.data.csv(
                'file://../../example_training_data/titanic_train.csv',
                csvConfig
            ),
            titanicMock,
            3
