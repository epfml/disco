...
export class IndexedDB extends Memory {
    ...
    infoFor(source: ModelSource): ModelInfo {
        ...
        const type =
            stringType === 'working' ? ModelType.WORKING : ModelType.SAVED
    ...
    async getModelMetadata(
        source: ModelSource
    ): Promise<tf.io.ModelArtifactsInfo | undefined> {
    ...
    async saveWorkingModel(source: ModelSource): Promise<Path> {
        ...
        const dst = this.pathFor(
            await this.duplicateSource({ ...src, type: ModelType.SAVED })
        )
        await tf.io.copyModel(
