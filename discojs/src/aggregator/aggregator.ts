 ...
 */
export abstract class Aggregator extends EventEmitter<{
  ...
  getPromiseForAggregation(): Promise<WeightsContainer> {
    return new Promise<WeightsContainer>((resolve) =>
      this.once("aggregation", resolve),
    );
  }
  ...
  add(
    ...
    if (!this.isValidContribution(nodeId, aggregationRound))
      throw new Error(
        "Tried adding an invalid contribution. Handle this case before calling add.",
      );
    ...
    this._add(
      nodeId,
      contribution,
      communicationRound ?? this.communicationRound,
    );
  ...
  isValidContribution(nodeId: client.NodeID, round: number): boolean {
    ...
    if (!this.isWithinRoundCutoff(round)) {
      debug(
        `Contribution rejected because round ${round} is not within round cutoff`,
      );
      return false;
    }
    return true;
  ...
  log(step: AggregationStep, from?: client.NodeID): void {
    switch (step) {
      case AggregationStep.ADD:
        debug(
          `Adding contribution from node ${from ?? '"unknown"'} for aggregation round ${this.round} and communication round ${this.communicationRound}`,
        );
        break;
      case AggregationStep.UPDATE:
        ...
        debug(
          `Updating contribution from node ${from} for aggregation round ${this.round} and communication round ${this.communicationRound}`,
        );
        break;
      case AggregationStep.AGGREGATE:
        debug(
          `Buffer is full. Aggregating weights for round aggregation round ${this.round} and communication round ${this.communicationRound}`,
        );
        break;
      default: {
        const _: never = step;
