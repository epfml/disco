...
export class DecentralizedClient extends Client<"decentralized"> {
  ...
  private async establishPeerConnections(): Promise<void> {
    if (this.server === undefined) {
      throw new Error(
        "peer's server is undefined, make sure to call `client.connect()` first",
      );
    }
    if (this.#pool === undefined) {
      throw new Error(
    ...
    try {
      debug(
        `[${shortenId(this.ownId)}] is waiting for peer list for round ${this.aggregator.round}`,
      );
    ...
    connections.forEach(async (connection, peerId) => {
      ...
      for (let r = 0; r < this.aggregator.communicationRounds; r++) {
        try {
          ...
          debug(
            `Error for [${shortenId(this.ownId)}] while receiving payloads: %o`,
            e,
          );
        }
      }
  ...
  private async exchangeWeightUpdates(
    weights: WeightsContainer,
  ): Promise<WeightsContainer> {
    if (this.aggregationResult === undefined) {
      throw new TypeError("aggregation result promise is undefined");
    ...
    for (
      ...
      try {
        result = await Promise.race([
          ...
          timeout(
            undefined,
            "Timeout waiting on the aggregation result promise to resolve",
          ),
        ]);
      } catch (e) {
