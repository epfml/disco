...
import { data, Logger, Memory, Task, TrainingInformation, TypedDataset } from '../index.js'
...
export class Disco {
  ...
  async *fit(data: data.DataSplit | TypedDataset): AsyncGenerator<RoundLogs & { participants: number }> {
    this.logger.success("Training started.");

    if (Array.isArray(data))
      data = await datasetToDataSplit(this.task, data)
