...
describe("text processing", () => {
  ...
  const shortExpectedTokens = [
    11748, 1391, 11160, 30642, 7509, 1782, 422, 44212, 87, 268, 10071, 14,
    35636, 364, 8172,
  ];

  it(
    "can tokenize text with the Llama 3 tokenizer",
    { timeout: 5_000 },
    async () => {
      const tokenizer = await Tokenizer.from_pretrained(
      ...
      const expectedTokens = [
        9906, 1917, 11, 264, 18399, 220, 16, 220, 11727, 20, 11, 32167, 220,
        25208, 13, 27685, 374, 264, 1949, 2262, 2930, 83708, 5439, 323, 18908,
        555, 264, 4029, 720, 315, 23872, 11, 3967, 439, 119234, 291, 5493, 13,
        78811, 555, 28933, 23782, 323, 30390, 328, 4091, 389, 6186, 220, 868,
        11, 220, 1049, 16, 11, 27685, 374, 21685, 555, 279, 90940, 5114, 11,
        459, 3778, 33184, 7471, 430, 51242, 264, 5687, 315, 927, 220, 7007,
        1274, 8032, 22, 60,
      ];
  ...
  it("pads sequence until enough token are generated", async () => {
    ...
    const tokens = tokenizer.tokenize(shortText, {
      padding: "left",
      max_length,
    });
    const paddedSequence = Repeat(
      paddingToken,
      max_length - shortExpectedTokens.length,
    )
      .concat(shortExpectedTokens)
      .toArray();
...
describe("Multi-Tokenizer Tests", { timeout: 20_000 }, () => {
  const sampleText =
    "Hello, world! This is a test string to check tokenization.";
  ...
  for (const name of tokenizerNames)
    it(`should tokenize text using tokenizer "${name}"`, async () => {
      ...
      for (const token of tokenArray) expect(token).to.be.a("number");
    });
});
...
describe("Encode-Decode tokenization", { timeout: 20_000 }, () => {
  it("should return text equal to the original after encode-decode tokenization using GPT2 tokenizer", async () => {
    ...
    const tokenizer = await Tokenizer.from_pretrained("Xenova/gpt2");
    const originalText =
      "Hello, world! This is a test for encode-decode tokenization.";
