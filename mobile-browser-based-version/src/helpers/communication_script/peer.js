...
export async function handleDataEnd(data, buffer) {
    ...
    switch(data.cmdCode) {
        case CMD_CODES.WEIGHT_REQUEST:
            ...
            const epochWeights = {epoch : buffer.lastUpdate.epoch, weights : buffer.lastUpdate.weights}
            console.log("Sending weights to: ", receiver)
            await sendData(epochWeights, CMD_CODES.AVG_WEIGHTS, buffer.peerjs, receiver)
            break
    }
}