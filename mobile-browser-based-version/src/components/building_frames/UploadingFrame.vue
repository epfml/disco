<template>
  ...
  <div v-else>
    <!-- Single class uploading process -->
    <div v-if="nbrLabels == 1">
      <!-- Upload File Card-->
      <div class="grid grid-cols-1 p-4 space-y-8 lg:gap-8">
        <div class="container mx-width lg h-full">
          <!-- Card header -->
          ...
          <SingleUploadFrame
            ...
            v-bind:fileUploadManager="fileUploadManager"
            v-bind:preview="true"
            v-bind:label="'Label ' + String(1)"
          />
        </div>
      </div>
    </div>

    <!-- Multi-class uploading process -->
    <div v-if="nbrLabels > 1">
      <div class="grid grid-cols-1 p-4 space-y-8 lg:gap-8">
        <div class="container mx-width lg h-full">
          <!-- Card header -->
          ...
          <div v-for="item in labels" :key="item">
            <SingleUploadFrame
              ...
              v-bind:label="'Label ' + String(item)"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    if (this.Task.trainingInformation.csvLabels) {
      this.csvLabels = true;
    } else {
