...
<script>
import {training_information, create_model} from "./MNIST_script"; // <-- Change import here

import * as tf from "@tensorflow/tfjs";
import { get_model_info } from "../../helpers/indexedDB_script";
...
export default {
  ...
  methods: {
    goToTraining() {
      if (!this.ready_to_train) {
        ...
        this.$router.push({ path: "/".concat(training_information.model_id).concat("/training")});
      }
    },
    async deleteModel() {
        console.log("Delete Model")
        this.saved_model_exists = false
        await tf.io.removeModel('indexeddb://saved_'.concat(training_information.model_id));
    },
    async optionNewModel() {
      ...
      if (this.choice_new_model) {
        ...
        this.$toast.success(
          "A new "
            .concat(training_information.model_id)
    ...
    async create_new_model() {
      ...
      await new_model.save(save_path_db);
    },
    getTheme: function () {
  ...
  async mounted() {
    ...
    this.$nextTick(async function () {
      let save_name = "saved_".concat(training_information.model_id);
