        ...
        <main class="flex-1">
          ...
          >
              ...
              <div class="container mx-width lg h-full">
                ...
                <div class="relative pb-4">
                  <article
                    ...
                    class="relative h-full flex flex-col bg-white rounded-lg dark:bg-darker"
                    v-on:drop="dropHandler(event)"
                    v-on:dragover="dragOverHandler(event)"
                    v-on:dragleave="dragLeaveHandler(event)"
                    v-on:dragenter="dragEnterHandler(event)"
                  >
                    <!-- scroll area -->
                    <section
                    ...
                    >
                      <header
                        v-on:click="onClickHandler(event)"
                        class="border-dashed border-2 border-gray-500 dark:border-primary py-12 flex flex-col justify-center items-center"
                      >
                        <p
                          class="mb-3 text-lg font-semibold dark:text-lightflex flex-wrap justify-center"
                        >
                        ...
                        <button
                          id="button"
                          class="mt-2 rounded-sm text-white px-3 py-1 transition-colors duration-200 bg-primary hover:text-primary hover:bg-primary-100 dark:hover:text-light dark:hover:bg-primary-dark dark:bg-dark focus:outline-none focus:bg-primary-100 dark:focus:bg-primary-dark focus:ring-primary-darker"
                        >
                          Upload files
                        </button>
                      </header>
                      ...
                      <ul id="gallery" class="flex flex-1 flex-wrap -m-1">
                        ...
                        >
                          ...
                          <span
                            v-if="!image_uploaded"
                            class="text-small text-gray-500 dark:text-lightflex"
            ...
            <div class="flex items-center justify-center p-4">
              <button
                v-on:click="testButtonOnClickHandler()"
                id="test-button"
...
export default {
  ...
  mounted() {
    (StateStore.isDark = this.getTheme()), (StateStore.color = this.getColor);
  },
  methods: {
    async test() {
      if (this.tensorflowModel == null) {
      ...
      let prediction = "";
      if (imgElement == null) {
        this.status("Empty image found :(");
      } else {
        prediction = await this.predict(imgElement);
      }
    ...
    async predict(imgElement) {
      ...
      let mobilenet = await tf.loadLayersModel(MOBILENET_MODEL_PATH);
      // this.status("Predicting...")
      // const result = await this.tensorflowModel.classify(imgElement)
