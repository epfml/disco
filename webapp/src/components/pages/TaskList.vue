  ...
  <div class="space-y-8 mt-4">
    ...
    <div v-else class="contents">
      ...
      >
        ...
        <div
          class="order-1 lg:order-2 px-4 pb-4 lg:p-0 lg:max-w-[300px] 2xl:max-w-[350px]"
        >
          <p
            class="text-xl font-bold text-heading-light dark:text-heading-dark"
          >
            What are <DISCOllaboratives />?
          </p>
          <p class="mt-4">
            <DISCOllaboratives /> are machine learning tasks, such as diagnosing
            COVID from ultrasounds or classifying hand written digits, that you
            can join to train and contribute to with your own data. To give you
            a sense of <DISCO />, we pre-defined some tasks along with some
            example datasets. The end goal of <DISCO /> is for users to create
            their own custom <DISCOllaborative /> and collaboratively train
            machine learning models.<br />
            By participating in a task, you can either choose to train a model
            with your own data only or join a collaborative training session
            with other users. If you want to bring your own collaborative task
            into <DISCO />, you can do so by creating
            <button
            ...
            <br /><br />
            <b
              >The data you connect is only used locally and is never uploaded
              or shared with anyone. Data always stays on your device.</b
            >
          </p>
        </div>
        <div
                ...
                <div class="flex flex-row justify-between flex-wrap">
                  ...
                  <div class="flex flex-row shrink-0 justify-end gap-1">
                    <div
                      ...
                      :style="{ backgroundColor: getSchemeColor(task) }"
                    >
                      <div class="text-xs font-semibold text-slate-900">
                        {{ task.trainingInformation.scheme.toUpperCase() }}
                      </div>
                    </div>
                    <div
                      class="px-2 py-1 rounded-md flex items-center"
                      :style="{ backgroundColor: getDataTypeColor(task) }"
                    >
                      <div class="text-xs font-semibold text-slate-900">
                        {{ task.dataType.toUpperCase() }}
                      </div>
                    </div>
                  </div>
                </div>
...
onMounted(() => {
  void tutorialStore.startOnFirstVisit();
});
</script>
