...
import { dataset, ConsoleLogger, training, TrainingSchemes, TrainingInformant, EmptyMemory, tasks, client, Weights, informant } from 'discojs'
...
describe('end to end', function () {
  ...
  async function cifar10user (): Promise<void> {
    ...
    const cli = await getClient(client.decentralized.Unsecure, server, cifar10)
    await cli.connect()
  ...
  async function makeClient (input: number[]): Promise<Weights> {
    ...
    const clientCurrent: client.decentralized.Unsecure = await getClient(client.decentralized.Unsecure, server, TASK)
  ...
  async function cifar10userSec (): Promise<void> {
    ...
    const cli = await getClient(client.decentralized.Secure, server, cifar10)
    await cli.connect()
  ...
  async function makeClientSecure (input: number[]): Promise<Weights> {
    const TASK = tasks.cifar10.task
    const clientCurrent = await getClient(client.decentralized.Secure, server, TASK)
    const weights: Weights = makeWeights(input)
    const trainingInformant1: TrainingInformant = new informant.DecentralizedInformant('0', 0)
    await clientCurrent.connect()
    return await clientCurrent.onRoundEndCommunication(weights, weights, 0, trainingInformant1)
