...
describe("end-to-end federated", () => {
	...
	it("observes tensor memory across federated training rounds", { timeout: 200_000 }, async () => {
	...
	it("releases federated client tensors when clients close", { timeout: 200_000 }, async () => {
		const baseTask = await defaultTasks.lusCovid.getTask();
		const task: Task<"image", "federated"> = {
			...baseTask,
			trainingInformation: {
